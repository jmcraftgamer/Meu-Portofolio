...
async function handleRequest(req) {
  ...
  if (!path.startsWith('/api/')) return fetch(req);

  if (method === 'OPTIONS') return new Response(null, { status: 200, headers: { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET,POST,PUT,DELETE,OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type,Authorization' } });
  ...
  try {
    ...
    if (path === '/api/auth/register' && method === 'POST') {
      if (users.find(function(u) { return u.email === body.email; }))
        return json(null, 400, { error: 'Email já cadastrado' });
      var u = { id: nextUserId++, name: body.name, email: body.email, password: body.password, phone: body.phone || '', company: body.company || '', isAdmin: body.email === 'admin@gmail.com' ? 1 : 0, createdAt: new Date().toISOString() };
      users.push(u);
      var payload = { id: u.id, email: u.email, name: u.name, isAdmin: u.isAdmin };
      var token = btoa(JSON.stringify(payload)) + '.' + btoa(JWT_SECRET);
      return json(null, 200, { token: token, user: { id: u.id, name: u.name, email: u.email, isAdmin: !!u.isAdmin } });
    }

    if (path === '/api/auth/login' && method === 'POST') {
      var u = users.find(function(u) { return u.email === body.email; });
      if (!u || u.password !== body.password) return json(null, 400, { error: 'Credenciais inválidas' });
      var payload = { id: u.id, email: u.email, name: u.name, isAdmin: u.isAdmin };
      var token = btoa(JSON.stringify(payload)) + '.' + btoa(JWT_SECRET);
      return json(null, 200, { token: token, user: { id: u.id, name: u.name, email: u.email, isAdmin: !!u.isAdmin } });
    }
    ...
    if (path === '/api/auth/me') {
      ...
      if (!userData) return json(null, 401, { error: 'Token required' });
      var u = users.find(function(u) { return u.id === userData.id; });
      if (!u) return json(null, 404, { error: 'User not found' });
      return json(null, 200, { id: u.id, name: u.name, email: u.email, phone: u.phone, company: u.company, isAdmin: u.isAdmin });
    }

    // PRODUCTS
    if (path === '/api/products' && method === 'GET')
      return json(null, 200, products.slice().reverse());

    if (path === '/api/products/promotions' && method === 'GET')
      return json(null, 200, products.filter(function(p) { return p.is_promotion === 1; }).reverse());

    var catMatch = path.match(/^\/api\/products\/category\/(.+)$/);
    if (catMatch && method === 'GET') {
      var cat = decodeURIComponent(catMatch[1]);
      if (VALID_CATEGORIES.indexOf(cat) === -1) return json(null, 400, { error: 'Categoria invalida' });
      return json(null, 200, products.filter(function(p) { return p.category === cat; }).reverse());
    }
    ...
    if (pidMatch && method === 'GET') {
      ...
      if (!p) return json(null, 404, { error: 'Produto nao encontrado' });
      return json(null, 200, p);
    }
    ...
    if (path === '/api/orders') {
      ...
      if (method === 'GET') {
        ...
        return json(null, 200, userOrders);
      }
    }

    // CHAT
    var chatMatch = path.match(/^\/api\/chat\/(\d+)$/);
    if (chatMatch && method === 'GET' || chatMatch && method === 'POST') {
      var userData = auth(req);
      if (!userData) return json(null, 401, { error: 'Token required' });
      var oid = parseInt(chatMatch[1]);
      var order = orders.find(function(o) { return o.id === oid; });
      if (!order) return json(null, 404, { error: 'Pedido não encontrado' });
      if (order.userId !== userData.id && !userData.isAdmin) return json(null, 403, { error: 'Acesso negado' });
      if (method === 'POST') {
    ...
    if (path === '/api/admin/orders' && method === 'GET') {
      var userData = auth(req);
      if (!userData) return json(null, 401, { error: 'Token required' });
      if (!userData.isAdmin) return json(null, 403, { error: 'Admin only' });
      ...
      return json(null, 200, adminOrders);
    }
    ...
    if (statusMatch && method === 'PUT') {
      ...
      if (!order) return json(null, 404, { error: 'Pedido nao encontrado' });
      order.status = body.status;
      return json(null, 200, order);
    }
    ...
    if (path === '/api/admin/stats' && method === 'GET') {
      ...
      if (!userData) return json(null, 401, { error: 'Token required' });
      if (!userData.isAdmin) return json(null, 403, { error: 'Admin only' });
      return json(null, 200, { totalOrders: orders.length, totalRevenue: orders.filter(function(o) { return o.status !== 'cancelled'; }).reduce(function(s, o) { return s + o.value; }, 0), pendingOrders: orders.filter(function(o) { return o.status === 'pending'; }).length, completedOrders: orders.filter(function(o) { return o.status === 'completed'; }).length, inProgressOrders: orders.filter(function(o) { return o.status === 'in_progress'; }).length, ordersByMonth: [], ordersByType: [] });
    }

    if (path === '/api/test')
      return json(null, 200, { status: 'ok', products: products.length, users: users.length });

    return json(null, 404, { error: 'Rota não encontrada' });
  } catch (e) {
    return json(null, 500, { error: e.message || 'Erro interno' });
  }
}
